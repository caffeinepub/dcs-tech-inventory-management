...
import type { BulkImportResult, BulkInventoryRecord } from "../backend";
...
function parseCsvText(text: string): ParseResult {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (!Number.isInteger(quantityNum) || quantityNum < 0) {
      skipped.push({
        rowIndex,
        reason: `quantity must be a non-negative integer (got: "${rawData.quantity}")`,
        rawData,
      });
      continue;
    }

    const stockThresholdNum =
      rawData.stockThreshold !== "" ? Number(rawData.stockThreshold) : 0;
    if (!Number.isInteger(stockThresholdNum) || stockThresholdNum < 0) {
      skipped.push({
        rowIndex,
        reason: `stockThreshold must be a non-negative integer (got: "${rawData.stockThreshold}")`,
        rawData,
      });
      continue;
    }
...
function downloadTemplate() {
  ...
  const example =
    "PN-1001-A,Widget A,A standard widget,50,10,Electronics,Warehouse A";
  const csv = `${header}\n${example}\n`;
...
export default function CsvImportSection() {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file.name.endsWith(".csv")) {
      ...
      setParseResult({
        valid: [],
        skipped: [],
        headerError: "Please upload a valid .csv file.",
      });
      return;
    }
  ...
  return (
    <div className="space-y-5">
      ...
      {/* File drop zone */}
      <label
        className="border-2 border-dashed border-border rounded-xl p-6 text-center cursor-pointer hover:border-primary/50 hover:bg-accent/20 transition-colors block"
        onKeyDown={(e) => {
      ...
      {parseResult && !parseResult.headerError && (
        <div className="space-y-4">
          ...
          <div className="flex flex-wrap gap-2 items-center">
            <Badge
              variant="outline"
              className="gap-1.5 text-xs bg-success/10 text-success border-success/20"
            >
            ...
            {clientSkippedCount > 0 && (
              <Badge
                variant="outline"
                className="gap-1.5 text-xs bg-warning/10 text-warning border-warning/20"
              >
                <AlertTriangle className="h-3 w-3" />
                {clientSkippedCount} skipped row
                {clientSkippedCount !== 1 ? "s" : ""}
              </Badge>
            )}
          </div>
          ...
          {validCount === 0 && (
            <Alert>
              ...
              <AlertDescription>
                All rows were skipped due to validation errors. Please fix the
      ...
      {parseResult && !parseResult.headerError && (
        <div className="flex gap-3">
          <Button
            ...
            disabled={
              bulkImport.isPending || validCount === 0 || !!importResult
            }
            className="gap-2"
          >
