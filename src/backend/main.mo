...
// Migration information: Added ?blobId to InventoryItem (was missing in initial deployment, that's why this whole update was necessary).
...
actor {
  ...
  public shared ({ caller }) func adjustInventory(
    ...
    // Staff or admin can adjust inventory
    if (not isStaffOrAdmin(caller)) {
      Runtime.trap("Unauthorized: Only staff or admins can adjust inventory");
    };
    
    // Staff must be approved (admins bypass approval)
    if (not AccessControl.isAdmin(accessControlState, caller) and not isApprovedUser(caller)) {
  ...
  public shared ({ caller }) func bulkImportInventory(records : [BulkInventoryRecord]) : async BulkImportResult {
    ...
    for (record in records.values()) {
      ...
      for ((id, item) in inventoryMap.entries()) {
        if (Text.equal(item.sku, record.partNumber)) {
          foundId := ?id;
        };
      };
  ...
  public shared ({ caller }) func setInventoryItemImage(itemId : Nat, blobId : Text) : async () {
    // Staff or admin can set images
    if (not isStaffOrAdmin(caller)) {
      Runtime.trap("Unauthorized: Only staff or admins can set item images");
    };
    
    // Staff must be approved (admins bypass approval)
    if (not AccessControl.isAdmin(accessControlState, caller) and not isApprovedUser(caller)) {
      Runtime.trap("Unauthorized: Only approved users can set item images");
    };
    
    switch (inventoryMap.get(itemId)) {
      case (null) { Runtime.trap("Item not found") };
      case (?item) {
        let updatedItem = { item with blobId = ?blobId };
        inventoryMap.add(itemId, updatedItem);
      };
    };
  };

  public query ({ caller }) func getInventoryItemImage(itemId : Nat) : async ?Text {
    if (not isApprovedUser(caller) and not AccessControl.isAdmin(accessControlState, caller)) {
      Runtime.trap("Unauthorized: Only approved users can view images");
    };
    switch (inventoryMap.get(itemId)) {
      case (null) { null };
      case (?item) { item.blobId };
    };
  };
};
